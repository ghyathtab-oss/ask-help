...
function onDutyResidents() {
  return residents.filter((resident) => resident.isOnDuty === true);
}

function residentCard(resident) {
  const isSelected = state.selectedIds.has(resident.id);
  return `
    <article class="resident-card ${isSelected ? 'selected' : ''}" style="--resident-color:${esc(resident.color)}">
...
function chooseResident(id, checked) {
  if (checked && !state.selectedIds.has(id) && state.selectedIds.size >= MAX_SELECTION) {
    showToast(`يمكن اختيار ${MAX_SELECTION} مقيمين كحد أقصى.`);
    renderHome();
    return;
  }
